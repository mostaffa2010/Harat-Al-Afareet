 ...
 * Central Game Orchestrator & State Machine (Version 5 — 10 Min Run & Freeze Buffer)
 */
...
export class Game {
    constructor() {
        ...
        // 0.5s brief freeze buffer after choosing an upgrade
        this.graceTimer = 0;
    ...
    handleUpgradeChosen(card) {
        ...
        // 0.5s brief freeze buffer so player sees their surroundings before monsters resume
        this.graceTimer = 0.5;
        if (this.player) {
            this.player.grantInvulnerability(1.0);
    ...
    triggerLevelUp() {
        ...
        // Always exactly 3 choices for all heroes
    ...
    update(dt) {
        if (this.state === GAME_STATES.PLAYING) {
            ...
            // Grace period: allow camera/visuals, but freeze simulation for 0.5s
